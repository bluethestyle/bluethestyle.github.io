---
title: "[Study Thread] PLE-4 — CGC 게이팅의 두 변형과 HMM Triple-Mode 라우팅"
...
PLE의 심장인 CGC 게이트의 두 변형 — 원본 논문의 CGCLayer(가중합)과
본 구현이 실제로 쓰는 CGCAttention(블록 스케일링) — 의 수식과 Expert
Collapse를 막는 정규화 기법들, 그리고 HMM 기반의 Triple-Mode 라우팅을
다룬다.*
...
RecSys 2020)의 **CGCLayer**는 각 Expert 출력을 "벡터 단위로 더해 섞는"
가중합 방식이었고, 본 구현의 **CGCAttention**은 Expert 출력 블록에
"스칼라 가중치를 곱해 크기만 조절하는" 블록 스케일링 방식이다. 이
4편은 두 변형의 차이, CGCAttention에 얹은 entropy 정규화와 차원
정규화, 그리고 그 뒤에 붙는 HMM Triple-Mode 라우팅을 다룬다.

### 변형 1 — CGCLayer: 벡터 가중합 (원본 PLE 논문)

원본 논문의 CGCLayer는 MMoE의 gate 구조를 그대로 따른다. 태스크별
gate가 Expert 출력의 가중합을 계산한다.
...
- **전제**: 모든 Expert가 *동일한 출력 차원*을 가져야 한다. 가중합이
  성립하려면 $\mathbf{h}_i^{expert}$ 들이 같은 $\mathbb{R}^d$ 에 살아야 하기 때문이다.
- **장점**: 수식이 깔끔하고, 태스크별로 "A Expert 70%, B Expert 30%"
  같은 자연스러운 혼합이 가능하다.
- **한계**: 본 프로젝트처럼 Expert 출력 차원이 이종(unified_hgcn 128D,
  나머지 perslay/temporal/deepfm/lightgcn 등 64D)인 경우 그대로 쓸 수
  없다. 모든 Expert를 동일 차원으로 맞추려면 정보 손실을 각오해야 한다.

### 변형 2 — CGCAttention: 블록 스케일링 (본 구현)
...
> 등 다양한 단위에 적용된 것이다. 본 구현의 CGC는 원 논문과 달리 블록
> 스케일링 + 차원 정규화를 적용하여 이종 Expert 차원 불일치를
> 처리한다.
