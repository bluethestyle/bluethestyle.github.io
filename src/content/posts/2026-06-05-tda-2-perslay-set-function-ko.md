...
프로덕션 추론은 **사전 계산 폴백** 을 돈다 — 오프라인 요약 피처를
3-layer MLP 에 통과시키는 경로다. 그 피처마저 없으면 Expert 는 배치를
깨뜨리는 대신 zero 벡터로 강등된다.

> **이 글 이후 숫자가 바뀌었다.** 집필 시점의 폴백 폭은 **70D**(short 24D
> + long 스키마 라벨 36D + phase 10D)였고 MLP 는 $70 \to 64 \to 64 \to 64$
> 였다. 2026-06-24 자로 PersLay Expert 의 `input_dim` 은 **58D** 로
> 정정됐다 — `tda_long` 의 실측 산출이 36D 가 아니라 **24D** 였기
> 때문이다(β₂/H₂ 부분이 생산되지 않음). 그래서 24 + 24 + 10 = **58**
> 이고, 차액 12D 는 Domain 그룹의 pad 로 남는다. 오프라인 블록을
> 스키마 라벨로 세면 70D, 실산출로 세면 58D 다.
