...
> **태스크 개수에 대한 주석.** "13개" 는 참조서가 18→13 으로 줄인 시점의 숫자다. 현재 온프렘 로스터는 설계 문서 기준 **16개**, 코드(`active_tasks.py`) 기준 17개 정의 / **활성 15개** 이고, 공개 AWS 벤치마크는 **12개** 로 정합됐다.
...
비슷한 방향을 보인다. 클러스터별 신호가 *결정 함수 전체를 바꿀 만큼*
...
데이터 낭비다 — 우리가 이미 아는 관계를 모델이 시간을 써서 처음부터 다시 찾는 셈이다.
...
원리 — "가져다 쓸 정보가 없으면 identity mapping 으로 수렴한다" 가 기본 동작이다.
...
> (예시: 이 도식은 온프렘 기술참조서의 일반 로스터를 쓴다. 공개 벤치마크는 **12개 태스크** 다.)
...
CTR (이진 분류), LTV (회귀), Brand_prediction (128개 브랜드 중 contrastive)
은 동일한 head 를 쓸 수 없다. Task Tower 는 공통 얕은 MLP 한 덩어리와
태스크 타입별 출력층의 조합으로 이 문제를 푼다.
