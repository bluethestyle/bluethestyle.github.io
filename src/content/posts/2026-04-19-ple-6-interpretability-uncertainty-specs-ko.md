...
Shared Expert 결합 표현(576D)에서 해석 가능한 sparse feature를
...
> 있는 특징을 추출할 수 있다는 점이다. 본 시스템의 SAE는 576D → 2304D
> 과완전 인코딩을 사용하여, 576차원에 뒤섞여 있는 Expert 정보를
> 2304개의 해석 가능한 단위로 "풀어헤치는" 역할을 한다.
...
> **SAE latent 활용.** `PLEClusterOutput.sae_latent` (2304D sparse
...
| 항목 | MMoE | 본 구현 |
|---|---|---|
| Expert 수 | 동일 구조 N개 | 이종 8개 (GCN, PersLay, DeepFM, ..., RawScale) |
...
| `shared_experts` | 8개 Expert 설정 | `_build_shared_experts` |
...
직관, PLE-3 의 입력 구조(PLEClusterInput · 734D)와 8개 이종 Shared
