...
- `KoreanFRIAAssessor` — AI 기본법 §35, 7-차원, 5년 보존
- `FRIAEvaluator` — EU AI Act Article 9, 5-차원, 지속 모니터링
...
- **인적 감독** — kill switch 발동 이력, HumanReviewQueue tier
  2/3 처리량, 상위 단계 보고 응답 시간.
- **개인정보** — PIPA §37의2 프로파일링 opt-out 처리율, 신용정보법
  §36의2 대응 기록.
- **책임성** — 모델 버전·학습 데이터 버전·config 버전의 추적
  가능성 (Ep 3 감사 테이블 조인으로 확인).
...
1. Ep 2 의 승격 게이트 통과
2. 승격 결정된 경우, `KoreanFRIAAssessor` 가 호출되어 7-차원 평가
   실행 → 해당 테이블에 저장
3. 병행하여 `FRIAEvaluator` 가 호출되어 5-차원 평가 → 별도 테이블에 저장
4. `AnnexIVMapper` 가 두 결과를 통합해 Article 11 기술문서용 리포트 생성
5. 감사 로그에 FRIA 완료 이벤트 엔트리

각 단계가 실패해도 파이프라인 자체는 중단되지 않는다 — 감사
인프라 실패가 운영 중단으로 번지는 걸 피하려는 best-effort 호출
(Ep 2 의 승격 감사 로깅과 같은 원칙). 대신 실패 사실이 감사 로그에
별도 엔트리로 남아 AuditAgent 가 야간에 발견한다.

## 금감원 쿼리의 답

"5년간 보존된 FRIA 결과를 제출하라" — 쿼리 하나. FRIA 결과 테이블
...
- **차원 scoring 의 실제 값은 사람이 결정한다.** "차별 차원 점수
  0.72 가 적절한가?" 에 답하는 건 담당 임원 결재선의 몫이다. 자동화되는
  건 점수 계산이지 점수 판정이 아니다.
