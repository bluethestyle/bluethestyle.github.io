...
(현재 오픈소스로 공개된 AWS 벤치마크 버전은 결정론적 리키지 및 중복 태스크를 제거하여 **12개 태스크** 와 349차원 입력 피처[Phase 0 후 403D]로 축소되어 있으나, 핵심 아키텍처와 엔지니어링 패턴은 동일하다. — 초기엔 13개였으나 AWS 저장소가 실행별 태스크 구성 provenance 에 맞춰 13→12 로 정합했다.)
