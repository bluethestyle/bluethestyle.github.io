...
*시리즈 "3개월간의 금융 AI 개발기" 1편. Claude Code를 보조 개발 도구로 삼고, GPU 한 대와 3명의 팀원으로 금융 추천 시스템을 구축한 과정을 다룬다.*
...
팀 구성원은 데이터 사이언티스트 겸 PM/리드 아키텍트 1명, 그리고 엔지니어 2명으로 총 3명이었다.

엔지니어 2명은 대학 졸업 후 취업을 준비 중인 청년 자문위원 자격의 서포터즈로, 정규 근로 계약 없이 프로젝트에 참여하는 인력이었다. 기관 차원의 대규모 인력이나 별도의 서버 자원 지원은 없는 상태로 프로젝트가 시작되었다.

하드웨어는 VRAM 12GB의 RTX 4070 소비자용 데스크톱 GPU 한 대가 전부였으며, 일반적인 사무 공간에 배치되어 있었다.

데이터 처리를 위해 GPU 서버 지원이나 HIVE에서 Spark/Impala로의 환경 전환 요청은 반려되었다. 따라서 기존 HIVE 환경에서 네트워크 대역폭 한계를 고려한 병렬 쿼리 로직을 직접 설계해야 했다.

Claude Code, Gemini, Cursor 등 AI 도구 구독료와 AWS 스팟 인스턴스, S3 스토리지 비용 등은 팀 내부에서 자체적으로 해결했다. 자원이 제한적인 환경이었으나, 이를 주어진 조건으로 받아들이고 대안을 모색했다.

## Claude Code를 통한 한계 보완

제한된 인원과 장비만으로 80개의 DAG가 포함된 시스템을 3개월 만에 구축하기 위해, 프로젝트 초기부터 Claude Code를 개발 과정에 적극적으로 도입했다.
