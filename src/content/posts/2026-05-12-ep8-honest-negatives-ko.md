---
title: "[4개월 개발기] 에피소드 8 — 정직한 실패의 기록(Honest Negative Results)과 다음 단계"
date: 2026-05-12 12:00:00 +0900
categories: [FinAI Build]
tags: [finai-build, adatt, gradsurgery, negative-results, financial-ai]
lang: ko
excerpt: "4개월의 기록. 13개 태스크 환경에서 무의미한(Null) 수치로 수렴해버린 adaTT, 심각한 VRAM 오버헤드로 기각된 GradSurgery, 집필 당시 진행 중이던 Paper 3, 그리고 2026년 4월 30일부터 수집이 시작된 실 프로덕션 메트릭의 현재 상태. 작동하지 않은 것들이 작동한 것들만큼이나 중요한 이유를 기록한다."
series: three-months
part: 8
...
## 집필을 마치고 발행을 미뤄둔 Paper 3 (Loss Dynamics)

에피소드 6에서 "Paper 3 (Loss Dynamics)의 핵심 아이디어가 바로 이 심각한 버그 사태에서 싹을 틔웠다"고 은근조용히 예고한 적이 있다. 그 거창했던 Paper 3는 이제 14개의 발견을 담아 본문까지 완성됐다 — MTL을 동질 태스크 영역 밖으로 밀어붙일 때 무엇이 '조용히' 실패하는가를 축으로 재구성했다.

이 논문이 파고드는 핵심 질문은 이것이다. **"손실 함수(Loss Function)의 미세한 동역학(Dynamics) 차이 하나가, 전체 모델 아키텍처에 대한 철학적 결론마저도 정반대로 뒤집어버릴 수 있는가?"** 에피소드 6에서 어렵게 경험했던 'Sigmoid-Softmax 성능 역전' 사태가 이 섬뜩한 질문에 대한 단 하나의, 그러나 너무나도 강력한 데이터 포인트다. Paper 3는 이 우연한 발견을 체계적인 과학으로 승화시키려는 야심 찬 시도다. 다양한 손실 가중치 기법(Uniform, Uncertainty, GradNorm, DWA 등)과 다채로운 게이트 구조(Softmax, Sigmoid, 단순 MoE 등)를 조합한 거대한 매트릭스 위에서, 우리가 그토록 확신했던 아키텍처적 결론들이 환경 변수에 따라 얼마나 나약하게 흔들리고 부서지는지를 낱낱이 해부하는 것이다.

실험 설계와 본문 집필은 모두 끝났다. 다만 Zenodo 발행은 운영 데이터가 도착하는 시점으로 일부러 미뤄두었다 — 그때 Paper 1, 2, 3를 함께 다시 올리기 때문에 아직 따로 링크할 DOI는 없다. 운영 수치가 나오면 이 블로그의 Study Thread 섹션에서 상세히 다룰 것을 약속한다.
