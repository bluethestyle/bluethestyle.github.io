...
성과 PLE 의 표현 수준 분리가 adaTT 의 loss 수준 재혼합으로 무효화
된다는 이중 설명이 붙었다.
...
이 재해석이 의미하는 것 — adaTT 가 13-task 규모에서 PLE 를 해치지
도 않고, 눈에 띄게 개선하지도 않는다. *null 효과*. 즉 이 데이터·
이 아키텍처 조합에서는 adaTT 가 *쓸 이유가 없다*.
...
vs NOTEARS 미결정 — 모두 같은 개발 패턴을 거쳐 드러났다:
...
4단계가 Claude Code 의 장기 컨텍스트가 가장 결정적이었던 지점이다.
...
Claude Code 는 3개월 일정 안에서 그 정직함의 *비용을 감당 가능하게*
만들었다.
...
이 블로그는 그 패턴을 조금이라도 깨려는 시도다. "3개월 간 2명이
시도했지만 작동 안 한 것" 이 다른 팀에게 *몇 주의 시간을 절약* 해
준다면, negative result 를 문서화하는 비용 대비 이익이 크다.
...
아니라 실수를 발견할 수 있는 것 — Ep 6 의 그 문장이 이 시리즈 전체
의 함축.
