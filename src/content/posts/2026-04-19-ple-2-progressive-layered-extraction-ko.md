...
(heterogeneous)으로 구성한다.** 7개의 Shared Expert 각각이 *구조적으로
...
이 결정이 아래 섹션의 모든 내용 — 7개 이종 Expert 의 구성, 원본
CGCLayer 위에 얹는 CGCAttention 2단계 블록 스케일링, 이종 차원
정규화 — 의 전제가 된다.
...
7개 Shared Expert는 각각 전혀 다른 도메인의 시각을 제공한다.
...
이들은 동일한 고객 데이터를 7가지 서로 다른 "렌즈"로 바라본 결과다.
...
> **왜 7개 Expert 모두 필요한가.** 7개 Expert는 동일 고객의 다른 측면을
...
- $\mathbf{w}_k \in \mathbb{R}^7$: 태스크 $k$가 7개 Expert에게 부여하는 *신뢰도 벡터*

> **비유 — 의료 진단 위원회.** 7명의 전문의(Expert)가 환자(입력
...
> Expert 수가 7개로 적어 sparse 선택 없이도 계산 효율이 유지된다.
...
**해석**: "현재 입력($\mathbf{h}_{shared}$)을 보고 7개 Expert 각각의
...
**확장 (본 프로젝트)**: PLE의 아이디어 위에 7개 이종 도메인
Expert(GCN, TDA, DeepFM, Temporal, Graph, Causal, OT),
...
| 원칙 | 구현 |
|---|---|
| 공유와 분리의 균형 | 7 Shared Expert + CGC gate + GroupTaskExpertBasket |
