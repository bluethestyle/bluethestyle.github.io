...
한 주간 스케줄된 재학습 Job 이 SageMaker 에서 종료된다. v147 이라는
챌린저 모델이 등록 대기 상태가 된다. 이 시점부터 고객이 새 추천을
...
감사 로그가 쓰여지고, 승격되면 다음 Lambda 요청부터 v147 이 서빙
된다. 사람이 깨어나기 전에 결정이 끝나 있다.
...
`_decide_promotion()` 함수 안으로 들어가 본다. v147 이 거쳐가는
순서다.

*1단계 — 운영자 오버라이드 체크.* `--force-promote` 플래그가 걸려
있는가. 이번에는 자동 스케줄 재학습이니 아니다. 다음으로 넘어간다.

*2단계 — 챔피언 존재 확인.* 레지스트리에 현 챔피언 v143 이 있다.
부트스트랩이 아니라 진짜 경쟁이 필요한 상황이다.

*3단계 — fidelity floor.* 증류된 student 모델이 teacher 대비 13개
태스크 각각에서 fidelity floor 를 지키는가. v147 은 모든 태스크에서
통과. 하나라도 실패했다면 여기서 바로 멈추고 거부됐을 것이다. 순서가
이렇게 배치된 이유는 뒤에서 다시 짚는다.

*4단계 — `ModelCompetition.evaluate()`.* v143 과 v147 의 학습 메트릭을
비교한다. 기준은 세 가지:

- 기본 지표 avg\_auc 가 0.5% 이상 개선됐는가. v147 의 0.731 vs v143 의
  0.724 — 개선폭 0.97%, 통과.
- 어떤 보조 지표도 2% 이상 퇴행하지 않았는가. 한 태스크에서 AUC 가
  0.8% 하락했으나 임계치 내. 통과.
- 개선이 통계적으로 유의한가. t-test p-value 0.012, 임계치 0.05 아래.
  통과.

세 조건 모두 통과. `promotion_approved=True` 를 받아 `registry.promote(
v147)` 이 호출된다. 감사 로그에 엔트리가 쓰인다 — champion\_version=
v143, challenger\_version=v147, decision=promote, reason=competition 결과
...
## v148 의 경우, 같은 게이트에서 멈춘다

두 주 뒤. v148 이 등록된다. 같은 `_decide_promotion()` 이 실행되지만
이번에는 다르게 끝난다.
...
메트릭만 보면 v148 의 avg\_auc 는 v147 보다 오히려 높다. 하지만
fidelity 검증이 *경쟁 이전에* 배치되어 있어서, 이 시점에서 거부가
확정된다. 4단계로 넘어가지 않는다.

감사 로그에 엔트리가 쓰인다 — decision=reject, reason="2 fidelity
failures: task\_churn (KL=0.31 > 0.20), task\_next\_best (KL=0.28 > 0.20)".
v148 은 레지스트리에 등록되되 (promoted=False 로) 운영에 들어가지
않는다.
...
v147 이 일주일째 운영 중이다. 화요일 오후 2시, 공정성 모니터가 경고를
띄운다. 특정 연령대 × 지역 조합에서 Disparate Impact 비율이 임계치를
하회. 다음 예정 재학습은 5일 뒤. 그때까지 기다릴 수 없다.

담당 엔지니어는 레지스트리에서 3주 전 known-good 상태의 v141 을 찾는다
(이전에 한 번 운영을 무사히 거쳤던 버전). 명시적으로 한 줄 실행:
...
"DI breach emergency rollback to v141", operator=엔지니어 ID.

2분 안에 v141 이 운영 중. 위원회는 그 주 금요일에 감사 엔트리를
받아 사후 검토한다. 검토의 내용은 "이 override 가 적절했나" 지,
"override 가 일어났는가" 가 아니다. 엔트리는 변경 불가능하게 기록됐고,
누가 언제 어떤 이유로 개입했는지가 hash chain 에 박혀있다.
...
v147 이 운영에 들어간 뒤에도 MRM 은 끝나지 않는다. 오히려 여기서
두 번째 루프가 시작된다.
...
판단이 위원회 일정에 의존하지 않는다. v148 이 월요일 새벽에 거부된
