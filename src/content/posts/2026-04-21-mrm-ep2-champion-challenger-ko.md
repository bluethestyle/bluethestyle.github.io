...
flowchart LR
  ...
  subgraph O [판정 결과 · 각 엔트리는 HMAC 감사 로그에 기록]
    direction TB
    d1([force_promote<br/>trigger=manual])
    d2([bootstrap<br/>trigger=auto])
    d3([reject<br/>fidelity 실패])
...
왼쪽 스파인의 어느 단계에서든 조건이 갈리면 오른쪽 결과 열로 빠져나와
판정이 확정되고, 그 판정이 HMAC 해시 체인 감사 로그에 엔트리로 남는다.
아래는 한 챌린저가 이 지도 위를 실제로 걸어가는 장면이다.
