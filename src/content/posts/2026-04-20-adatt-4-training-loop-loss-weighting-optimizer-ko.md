...
Expert 파라미터에 대해 계산되는데, 그게 frozen 이면 gradient 가 0 이므로
`autograd.grad` 호출은 낭비다.

왜 이렇게 나누는가. 클러스터별 학습을 Shared 와 같이 돌리면 클러스터
...
중요한 것은 adaTT 와의 *순서* 다. Uncertainty Weighting 은 adaTT *이전* 에
적용된다. 즉 adaTT 의 `task_losses` 입력에는 이미 불확실성 가중치가
반영된 값이 들어온다. 의도된 동작이다 — 비즈니스적으로 중요한
...
해결은 Expert 별 param_group 이다. 각 Shared Expert 의 파라미터를 별도
...
positive transfer" 라고 측정해 고정해둔 가중치는 이제 잘못된
관계를 반영하게 된다. gate dynamic 과 transfer dynamic 이 *둘 다* 멈춰야
수렴이 깨끗하다.
...
- Phase 2 시작 시 동일 처리. Shared 가 frozen 인데 CGC 만 계속 학습하면
  입력 (Expert 출력) 이 변하지 않으므로 gating 이 과적합으로 흐른다.
...
그대로다. `math.isfinite(loss_val)` 체크로 NaN / Inf loss 발생 시 해당
배치를 스킵하고 `optimizer.zero_grad()` 로 오염된 gradient 를 초기화한다.
OOM 은 `trainer.py` 의 예외 핸들러에서 배치 스킵으로 처리한다.
...
lr 과 SequentialLR 이 학습률을 Expert 별로 분배한다. CGC-adaTT 동기화
freeze 가 수렴을 정리하고, `grad_interval=10` 과 TF32 가 성능을 유지하며,
...
여기까지가 adaTT 서브스레드의 끝이다. 각 편은 이전 편의 결정이 남긴
문제에서 출발하는 사슬로 이어진다.
