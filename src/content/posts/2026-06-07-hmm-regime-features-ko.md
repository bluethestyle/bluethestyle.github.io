...
기재한다 — 늘어난 여섯 번째 상태만큼 메타 블록을 줄인다는 설명이다.

> **여기서 참조서와 코드가 갈린다.** `hmm_features.py` 의 차원 검증은
> 모드 구분 없이 `expected_dim = self.n_states + 5 + 6` 하나만 쓴다 —
> Behavior 의 메타 블록을 4D 로 줄이는 분기가 코드에 없다. 그 식대로면
> Behavior 는 $6 + 5 + 6 = 17$D 다. 반면 소비자 쪽 계약
> (`task_feature_mapper.py`)은 `hmm_behavior: 16` 으로 선언한다.
> 즉 **산출측 17 / 계약측 16 이 어긋나 있고**, 참조서의 "$6+4+6$" 은
> 그 어긋남을 사후 설명한 것에 가깝다. 세 모드 합 48D 라는 계약값은
> 그대로지만, Behavior 한 모드의 내부 분할은 코드에서 확인되지 않는다.
