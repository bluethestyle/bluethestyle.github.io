...
- **PIA (Privacy Impact Assessment)** — 개인정보보호법(PIPA) 제33조 · GDPR Article 35 가 요구하는 개인정보 영향평가. AI 가 학습·추론에 개인정보를 쓰는 한 모든 금융 기관이 작성 의무를 진다.
- **공시 리포트 (Public Disclosure)** — 금융위원회가 요구하는 분기 단위 공시. AI 시스템의 투명성, 모델 성능, 공정성 지표, 인시던트 요약, 고객 영향 평가를 정기적으로 공개해야 한다.
...
## Public Disclosure Generator — 금융위 공시용 분기 리포트

금융위원회의 공시 요구는 별개의 산출물이다. 5개 섹션을 정기적으로 공개해야 한다.
...
금융위원회가 분기 말에 공시 자료를 요청하면, 사람은 JSON 이 올바르게 생성됐는지만 확인하고 제출 버튼을 누른다. 숫자 자체를 타이핑하는 단계는 구조적으로 제거된다.
