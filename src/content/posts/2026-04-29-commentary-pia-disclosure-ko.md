...
- **공시 리포트 (Public Disclosure)** — 금융위원회의 AI 금융 서비스 관련 가이드라인 흐름에 맞춰 내부적으로 정례화한 분기 공시 체계. AI 시스템 투명성, 모델 성능, 공정성 지표, 인시던트 요약, 고객 영향, 데이터 거버넌스, 컴플라이언스 현황, AI 공시를 정기적으로 집계한다. 현재로선 법정 의무 조항이라기보다 *감독 당국의 기대에 대한 선제적 대응*에 가까우며, 규제가 구체화되면 그대로 확장 가능하도록 섹션을 8개로 모듈화해 두었다.
...
## Public Disclosure Generator — 분기 공시용 8개 섹션 리포트

위의 PIA 와는 별개의 산출물이다. 감독 당국의 기대와 금융위 AI 금융 서비스 가이드라인 흐름에 맞춰, 다음 8개 섹션을 정기적으로 집계하고 공개한다.
...
6. **데이터 거버넌스** — 데이터 출처, 보존 기간, 처리의 법적 근거
7. **컴플라이언스 현황** — 주요 규제 프레임워크(EU AI Act, GDPR, PIPA, AI 기본법) 대비 매핑 현황
8. **AI 공시 성명** — 추천에 AI 시스템이 사용된다는 공식 고지와 고객의 대응 방법

`PublicDisclosureGenerator` 는 이 여덟 섹션 각각에 대응하는 집계 쿼리를 분기 주기로 실행한다. 입력은 Ep 3 의 `log_operation`, `log_model_inference`, `log_attribution`, `log_guardrail` 그리고 Ep 6 의 공정성 아카이브다. 출력은 두 가지 형식으로 나간다 — **JSON** (기계 판독용, 감독 당국의 자동화된 수집 경로) 과 **Markdown** (사람 판독용, 공시 홈페이지와 PDF 변환용). S3 에 버전 관리로 저장된다.
...
- 공시 리포트의 여덟 섹션 구성 자체가 현재 감독 당국의 기대에 부합하는가 — 새로운 항목(예: 탄소 발자국, 공급망 위험) 이 추가되면 리포트 구조도 확장되어야 한다.
...
- `PublicDisclosureGenerator` — 금융위 분기 공시 (8섹션)
