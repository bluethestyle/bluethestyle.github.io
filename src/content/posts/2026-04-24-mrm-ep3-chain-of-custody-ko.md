...
*"MRM 스레드" 3편. Ep 2 가 승격 결정의 기록을 다뤘다면, 이번 편은 그 기록이 *무엇을* 담는지(7개 감사 테이블 · HMAC 체인), 그 기록을 *누가* 검증하는지(Ops · Audit 에이전트), 그리고 그 검증자들이 *어떻게 서로를 견제하는지*(3인 병렬 투표 · 2-Round 심의)까지 한 층씩 내려가 본다. 문서화와 감사는 늘 MRM의 핵심 기둥이었다 — ML/DL 시대에 달라지는 것은 감사 로그가 *주기적 기록* 이 아니라 *질의 가능한 기반(substrate)* 이 되어야 한다는 점이다.*
...
> 업데이트: 집필 당시 온프렘 스택은 이 14B Q4 모델을 썼다. 이후 내부
> 4-시나리오 벤치마크를 거쳐 Exaone 3.5 2.4B(합의) + Qwen 2.5 7B(도구
> 사용)로 교체됐다. 아래 세션 수치는 그 시점의 14B Q4 구성을 기준으로 한다.
