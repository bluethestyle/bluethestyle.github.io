...
비정상적으로 작아져 다른 태스크 학습에 사실상 기여하지 못하고, (b) 전이
학습의 평가 지표가 오염된다.
...
많이 exploit 하는가" 를 측정하게 된다. 실데이터에서 재현되지 않는
결과를 논문에 싣는 지름길이다.

*데이터 무결성 사냥은 매력 없는 작업이다.* 새 알고리즘을 시도하는
게 아니라 있는 데이터를 *의심* 하는 일이라, 며칠이 지나도 눈에 보이는
진척이 없다. 3인 팀의 시간 압박에서는 "빨리 결과 보고 싶다"
는 유혹과 싸우는 게 가장 어렵다. 이 유혹을 이긴 건 *이전 leakage
발견이 AUC 를 얼마나 끌어내렸는지에 대한 기억* 이다 — 0.87 → 0.62 의
...
Ep 3 에서 CLAUDE.md 를 "헌법" 이라 부른 이유가 여기 있다. 새 실험마다 위 조항을 일일이 떠올릴 필요가 없고, 새 팀원이 합류해도 즉시 적용된다.
