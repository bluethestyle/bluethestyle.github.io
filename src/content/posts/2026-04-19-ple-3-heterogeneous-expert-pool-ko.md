...
> **쌍곡 공간의 핵심 — {7,3} 타일링으로 본 Poincaré disk.** 위 그림은 7각형 7개가 한 점에서 만나지 못하므로(플랫 평면에서는 6개만 가능) **쌍곡 평면에서만** 존재하는 정규 타일링. 변은 경계에 수직으로 만나는 geodesic (쌍곡 "직선") 원호다. 평면에서는 반지름 $r$ 원 둘레가 $2\pi r$ 로 자라지만 쌍곡 평면에서는 $\sinh(r)$ 로 **지수적** 성장 — 그래서 경계 쪽에서 타일이 무한히 작아 보이지만, 쌍곡 거리로는 모두 같은 크기다. 트리 구조도 같은 이치로 "자리가 부족하지 않다". *이미지: [cduck/hyperbolic](https://github.com/cduck/hyperbolic) (MIT) polyTile7-3.svg*
