...
> **쌍곡 기하 도식 — (a) Poincaré disk 모델 + (b) geodesic 예시.** (a) 단위 원 내부 $\mathbb{D}$ 전체가 무한 쌍곡 평면 하나와 등가. 경계 $\partial\mathbb{D}$ 는 "무한히 먼 점들" 의 집합. (b) 쌍곡 *직선* (geodesic) 은 두 가지 형태 — 중심을 지나는 지름, 또는 경계에 **직각** 으로 만나는 원호. 평면에서는 반지름 $r$ 원 둘레가 $2\pi r$ 인데 쌍곡 평면에서는 $\sinh(r)$ 로 지수적으로 자라서, 트리 구조처럼 자식 수가 깊이마다 기하급수로 늘어나는 데이터가 "자리가 부족하지 않게" 들어간다. *도식 레이아웃은 Shi et al. (Medical Image Analysis 2016, PMC5099092) Figure 1 을 참고해 재현.*
