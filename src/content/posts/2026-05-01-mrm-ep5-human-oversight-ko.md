...
모니터에서 보호군 위반이 표면화된다 — 예를 들어 65세 이상 연령층의
...
프로세스* 다 — "매월 MRM 위원회에서 모니터링 출력을 검토합니다",
...
## HumanReviewQueue — Tier 2/3 자동 에스컬레이션
...
하고 분류하지 않는다 — 위 세 조건 중 하나가 맞으면 자동 tier 올라감.
즉 tier 체계는 규칙 집합 (`core/compliance/tier_evaluator.py`) 이고,
그 규칙 자체가 MRM 위원회의 심사 대상이 된다.

## auto_promote=false — Production Posture

Ep 2 에서 다룬 `_decide_promotion()` 은 모든 조건 통과 시 자동 승격
한다. *기술적으로* 는. 하지만 production 에서는 `pipeline.yaml`
의 `serving.competition.auto_promote: false` 가 강제되어 있어서,
모든 챌린저가 metric 통과해도 자동 승격되지 않고 `--force-promote`
운영자 override 없이는 현 챔피언이 유지된다.

이게 왜 필요한가 — *자동 승격이 가능한 조건* 은 기술적으로 모든
gate 가 구조적 불변성으로 작동한다는 가정 위에 있다. 현실에서는 그
...
`core/serving/fallback_router.py` 에 3 계층 서빙 폴백이 있다:

- **Layer 1** — PLE → LGBM 증류 모델 (일반 경로, 99%+ 트래픽)
- **Layer 2** — 증류 실패 시 LGBM 직접 학습 모델
- **Layer 3** — 금융 DNA 기반 룰 (모델 전체 실패 시)

2026 상반기에 Layer 4 가 추가됐다 — **인적 폴백**. `serving.review.
tier_3_human_fallback: true` 일 때만 활성. Layer 1-3 모두 신뢰도
...
§7 "Human-in-the-Loop 설계". 구현은 `core/serving/fallback_router.py`,
`core/compliance/human_review_queue.py`, `scripts/kill_switch.py`.
