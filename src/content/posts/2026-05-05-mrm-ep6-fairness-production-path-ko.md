...
매 윈도의 15개 지표가 S3 Parquet 에 쌓인다. 경로 구조 —
`fairness_archive_v1/year={yyyy}/month={mm}/day={dd}/attribute={attr}/metric={metric}/`.
압축 snappy, append-only.
...
hypothetical 공정성을 재구성 가능. `core/evaluation/counterfactual.py`
의 `CounterfactualEvaluator.from_config(serving.counterfactual_cc)` 가
estimator (ips/snips), min_lift, bootstrap CI 설정을 config 에서
읽어 실행.
...
결정은 여전히 공정성 위원회의 일이고, `pipeline.yaml` 의
`monitoring.fairness.thresholds` 에서 config-driven 으로 관리.

임계치 변경은 반드시 회의록 → PR → review → merge 경로. 조용한
config 변경은 `log_operation(event="threshold_change", ...)` 로
감사 로그에 자동 기록되어 누가 언제 왜 변경했는지 추적 가능.

이 구조의 핵심 — *임계치 결정 자체가 감사 대상* 이다. 단순히 "임계치
위반이 있었는가" 만이 아니라 "이 임계치가 합리적인가" 도 분기 위원회
의 심사 주제가 된다.
...
*약 48분* — OpsAgent 가 체인을 지켜보다가 MRM 위원회 주례 검토용
incident ticket 초안 작성, 관련 감사 엔트리로 링크.

금감원의 72시간 서면 보고 window 한참 안에서 위원회는 *사후 재구축된
...
6편에 걸쳐 "MRM 을 사후 보고서에서 구조적 불변성으로" 의 구체적
모습을 다뤘다 — 아키텍처 (Ep 1), 승격 게이트 (Ep 2), 감사 체인
(Ep 3), FRIA (Ep 4), 인간 감독 (Ep 5), 공정성 (Ep 6).

관통하는 패턴 — *규제 대응이 코드 경로의 부산물이 될 때* 가장 강하다.
문서로 증명하려 하면 문서가 코드와 drift 하면서 실효성 잃고, 위원회
회의로 덮으려 하면 회의 주기가 사건 주기와 어긋난다. 핵심 경로에
...
게 아니라 *이미 하는 일이 규제 요구에 응답하도록 구조* 가 배열된 것.

여전히 사람의 일 — 임계치 결정, 설계 심사, 사건 복구 판단, 법 개정
대응. MRM 위원회가 *심사하는 대상* 이 리포트에서 코드 경로로 이동
했을 뿐, 역할 자체가 사라진 건 아니다. 감독 당국이 준수를 *아키텍처
적으로 가독 가능* 하길 기대하는 시대에, 이 이동이 자연스러운 정렬
이다.
...
의 `core/monitoring/fairness_monitor.py`,
`core/evaluation/counterfactual.py`, `configs/pipeline.yaml` 의
`monitoring.fairness` 섹션.
